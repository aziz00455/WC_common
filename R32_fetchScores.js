...
(function () {
  ...
  async function requestKnockoutSync(syncUrl, payload = {}, options = {}) {
    const { minGapMs = 30 * 1000, force = false } = options || {};
    ...
    const res = await fetch(syncUrl, {
      ...
      body: JSON.stringify({ requestedAtUtc: toIsoNow(), ...payload })
    });
  ...
  function findEventByKickoff(events, match) {
    ...
    for (const event of events || []) {
      ...
      if (eventUtc && eventUtc === target) return event;
    }
    return null;
  }
      ...
      if (Array.isArray(source)) {
        for (const team of source) {
          ...
          const aliases = [canonical, team.shortName, team.abbreviation, team.code, team.espn]
            .map(normalizeText)
            .filter(Boolean);
          if (aliases.some(alias => normalizeKey(alias) === key)) return canonical || raw;
        }
  ...
  function extractEventDataForMatch(event, snapshotMatch) {
    ...
    if (currentTeam1 || currentTeam2) {
      ...
      if (idx1 === 1 && idx2 !== 0) {
        side1 = ordered[1]; side2 = ordered[0];
    ...
    return { team1, team2, score1, score2, status, gameTime, winner };
  }
  ...
  function shouldFetchScore(match, snapshotMatch, nowUtcMs) {
    ...
    if (snapshotMatch && snapshotMatch.status === "complete" && snapshotMatch.score1 != null && snapshotMatch.score2 != null) {
      return false;
    }
    return true;
  }
  ...
  async function fetchEventDataForMatch(match, snapshotMatch, nowUtcMs) {
    ...
    if (!shouldTryTeams && !shouldTryScores) return null;
  ...
  async function syncKnockoutSnapshotClient(db, options = {}) {
    ...
    return { ok: true, changedMatchCount, fetchedAtClientUtc: toIsoNow() };
  }

  async function loadR32Snapshot(db, options = {}) {
    const { syncUrl = "", triggerSyncFirst = false, syncPayload = {}, syncOptions = {} } = options || {};
    ...
    const snapshot = await loadKnockoutSnapshot(db);
    return { snapshot, fetchedAtClientUtc: toIsoNow() };
  }
  ...
  function startR32AutoRefresh(config) {
    ...
    timer = setInterval(() => { refresh({ isInitial: false }).catch(() => {}); }, refreshMs);
